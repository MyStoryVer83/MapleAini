...
function action(mode, type, selection) {
	...
	if (status == 0 && mode == 0) {
		...
		cm.sendNext("是吗……看来我的预感是错的，你好像没什么朋友啊？或者身上没有24万金币？如果你改变了主意，可以再来找我。等你有了钱的时候……呵呵……");
		cm.dispose();
		return;
	}	
	if (mode == 1)
	...
	if (status == 0) {
		...
		cm.sendYesNo("好的！明智的决定。价格不贵，因为我下定决心，给你打了个#r大折扣#k.#b好友目录添加5名一共是24万金币#k。当然，绝不零售。只要购买一次，目录就可以永久增加。对好友目录不足的人来说，这个买卖应该不坏。怎么样？你愿意支付24万金币吗？");
	} else if (status == 2) {
		var capacity = cm.getPlayer().getBuddylist().getCapacity();
		if (capacity >= 50 || cm.getMeso() < 240000){
			cm.sendNext("你……确定自己有#b24万金币#k吗？如果有的话，请你确认一下好友目录是否已经增加到最大了。即使钱再多，好友目录的人数也无法增加到#b50个以上#k。");
            cm.dispose();
		} else {
